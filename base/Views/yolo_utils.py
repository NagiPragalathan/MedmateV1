...
def weights_download(out='models/yolov3.weights'):
    yolo_path = os.path.join(settings.BASE_DIR,'base', 'yolov3.weights')
    
    _ = wget.download(yolo_path, out='models/yolov3.weights')

from MedmateV1.settings import BASE_DIR
import os
    
# weights_download() # to download weights
yolo = YoloV3()
load_darknet_weights(yolo,  os.path.join(BASE_DIR,'yolov3.weights')) 
